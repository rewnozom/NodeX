...
class TestChatPage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_parent.config.get_agent_config.return_value = {
            'developer': {'enabled': False, 'profile': {'name': 'DevAgent', 'config': {'current_workflow': 'develop'}}},
            'crew': {'enabled': False, 'profile': {'name': 'CrewAgent', 'config': {'current_workflow': 'design'}}}
        }

        # Initialize the Page instance with the mock parent
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.page.current_model_name, self.page.config.CHAT_MODEL, "Current model name should match config.")
    ...
    def test_send_message_valid(self):
        ...
        with patch.object(self.page, 'display_user_message') as mock_display_user, \
             ...
             patch.object(self.page, 'prepare_messages') as mock_prepare_messages, \
             patch.object(self.page, 'update_status_message') as mock_update_status, \
             patch('ai_agent.services.ai_service.AIService.get_response') as mock_get_response, \
             patch('ai_agent.threads.worker_thread.WorkerThread.start') as mock_worker_start:
            
            mock_prepare_messages.return_value = [{'role': 'system', 'content': 'Test system prompt'}, {'role': 'user', 'content': test_message}]
    ...
    def test_handle_ai_response_with_code_blocks(self):
        ...
        with patch.object(self.page, 'display_ai_message') as mock_display_ai, \
            ...
            mock_add_history.assert_called_once_with(self.page.current_chat_id, self.page.chats, {'role': 'assistant', 'content': "Here is some code:"})
            mock_save_chat_log.assert_called_once_with(self.page.current_chat_id, self.page.chats[self.page.current_chat_id])
    ...
    def test_handle_ai_response_no_code_blocks(self):
        ...
        with patch.object(self.page, 'display_ai_message') as mock_display_ai, \
            ...
            mock_add_history.assert_called_once_with(self.page.current_chat_id, self.page.chats, {'role': 'assistant', 'content': response_content})
            mock_save_chat_log.assert_called_once_with(self.page.current_chat_id, self.page.chats[self.page.current_chat_id])
            mock_update_status.assert_called_once_with("Response received", is_agent_message=True)
    ...
    def test_clear_chat_with_active_chat(self):
        ...
        with patch.object(self.page, 'save_chat_log') as mock_save_chat_log, \
            ...
            # Assuming setMarkdown is correctly called
            # You might need to adjust the patch path based on actual implementation
    ...
    def test_clear_chat_no_active_chat(self):
        ...
        with patch.object(QMessageBox, 'warning') as mock_warning:
            self.page.clear_chat()
            mock_warning.assert_called_once_with(self.page, "No Chat Selected", "No chat session is currently active.")
            logger.debug("Warning shown when trying to clear chat with no active session.")
