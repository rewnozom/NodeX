...
class HeaderSection(QWidget):
    ...
    def open_whisper(self) -> None:
        """Open the whisper window."""
        try:
            if not hasattr(self.parent, 'whisper_instance'):
                whisper_hub = WhisperHub()
                whisper_tab = whisper_hub.whisper_tab
                self.parent.whisper_instance = whisper_tab
            
            if self.parent.stacked_widget.indexOf(self.parent.whisper_instance) == -1:
                self.parent.stacked_widget.addWidget(self.parent.whisper_instance)
                
            self.parent.stacked_widget.setCurrentWidget(self.parent.whisper_instance)
            logger.info("Whisper window opened")
            
        except Exception as e:
            logger.error(f"Error opening whisper window: {e}")
            raise ActionError(f"Failed to open whisper window: {str(e)}")


    def open_terminal(self) -> None:
        """Open the terminal window."""
        logger.info("Terminal window opened")
        self._handle_button_action("terminal")
