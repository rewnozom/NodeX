...
def get_model(model_key, temperature=None, api_key=None):
    for company, models in MODELS.items():
        if model_key in models:
            ...
            if company == "Anthropic":
                ...
                return ChatOpenAI(api_key=api_key, base_url="https://openrouter.ai/api/v1", model=model_name, temperature=temp)
    ...
    if model_key in EMBEDDING_MODELS:
        if model_key == "HuggingFace Embeddings":
            ...
            return AzureOpenAIEmbeddings(deployment_name=model_key, api_key=api_key, azure_endpoint=azure_endpoint)
