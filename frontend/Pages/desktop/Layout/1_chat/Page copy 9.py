...
class Page(QWidget):
    ...
    def toggle_agent_mode(self, state):
        ...
        try:
            # Get current config but maintain new enabled state
            current_config = self.config.get_agent_config()
            current_workflow = current_config.get('config', {}).get('current_workflow')

            settings = {}
            if enabled:
                settings['current_workflow'] = current_workflow or self.workflow_combobox.currentText().split(" - ")[0].lower()

            # Important: Pass enabled state explicitly
            success = self.ai_service.update_agent(
            ...
            if success:
                ...
                workflow_text = f"Workflow: {settings.get('current_workflow', 'None') if enabled else 'None'}"
                ...
                QMessageBox.warning(self, "Agent Mode Error", "Failed to update agent mode")
