...
class Page(QWidget):
    ...
    def init_chat_history_tab(self):
        ...
        buttons_layout = QVBoxLayout()  # Byt till en vertikal layout
        ...
        # Gör knapparna större och mer läsbara
        for button in [self.load_chat_button, self.new_chat_button, self.rename_chat_button, self.delete_chat_button, self.clear_chat_button]:
            ...
            button.setMinimumHeight(30)  # Sätter en minimumhöjd för läsbarhet
    ...
    def toggle_agent_mode(self, state):
        ...
        """
        # Debug: skriv ut till logg/console vilket state vi får
        logger.info(f"toggle_agent_mode called with state: {state}")

        # Om state == Qt.Checked (vilket oftast är värdet 2) så ska agenten aktiveras
        if state == Qt.Checked:
            enabled = True
        ...
        # Uppdatera agent-konfigurationen via config
        self.config.update_agent_config({
            'enabled': enabled,
            'current_agent': current_agent
        })

        # Uppdatera GUI-status: ändra text på agent-status och workflow-comboboxens tillgänglighet
        if enabled:
            self.agent_status_label.setText("Agent: Enabled")
            # Aktivera workflow-menyn så att man kan ändra workflows
        ...
        # Eventuell ytterligare UI-uppdatering kan göras här
    ...
    def update_status_message(self, message, is_agent_message=False):
        ...
        Args:
            ...
            is_agent_message (bool): If True, update the agent status label instead of the general status label.
        """
        if is_agent_message:
            self.agent_status_label.setText(message)
        else:
