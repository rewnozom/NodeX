...
class Page(QWidget):
    ...
    def init_ui(self):
        ...
        # Set workflow combobox state based on agent configuration
        self.workflow_combobox.setEnabled(agent_config['enabled'])
        
        # If agent is enabled and has a current workflow, select it
        if agent_config['enabled'] and agent_config['config'].get('current_workflow'):
            current_workflow = agent_config['config']['current_workflow']
            # Find and select the matching workflow in the combobox
            for i in range(self.workflow_combobox.count()):
                if self.workflow_combobox.itemText(i).lower().startswith(current_workflow):
                    self.workflow_combobox.setCurrentIndex(i)
                    break
        
        # Connect workflow selection change
        ...
        if agent_config['enabled'] and agent_config['config'].get('current_workflow'):
            current_workflow = agent_config['config']['current_workflow']
            self.workflow_status.setText(f"Workflow: {current_workflow}")
            self.workflow_status.setStyleSheet("color: green;")
        else:
            self.workflow_status.setText("Workflow: None")
            self.workflow_status.setStyleSheet("color: gray;")
        self.workflow_status.setObjectName("workflowStatus")

        # Update agent status label
        self.agent_status_label.setText(f"Agent: {'Enabled' if agent_config['enabled'] else 'Disabled'}")
    ...
    def init_controls(self):
        ...
        self.agent_toggle = QCheckBox()
        self.agent_toggle.setChecked(self.config.get_agent_config()['enabled'])
        self.agent_toggle.stateChanged.connect(self.toggle_agent_mode)
        self.agent_toggle.setObjectName("agentToggle")
        ...
        # Workflow Dropdown ska vara aktiverad om Developer Mode är på
        self.workflow_combobox.setEnabled(self.config.get_agent_config()['enabled'])
    ...
    def update_workflow(self, workflow_text):
        ...
        if not agent_config['enabled']:
            logger.warning("Attempted to update workflow while agent is disabled")
            self.workflow_status.setText("Workflow: Disabled")
            self.workflow_status.setStyleSheet("color: gray;")
            return
        ...
        try:
            ...
            # Update agent configuration with new workflow
            ...
            if success:
                ...
                logger.info(f"Successfully updated workflow to: {workflow}")
            else:
                ...
                QMessageBox.warning(self, "Workflow Error", 
                                "Failed to update workflow. Check the logs for details.")
    ...
    def toggle_agent_mode(self, state):
        ...
        try:
            ...
            current_workflow = current_config.get('config', {}).get('current_workflow')

            # Update agent configuration while preserving workflow settings
            success = self.ai_service.update_agent(
                agent_type='developer' if enabled else 'chat',
                enabled=enabled,
                settings={'current_workflow': current_workflow} if current_workflow else None
            )

            if success:
                # Update GUI elements based on the new state
                self.workflow_combobox.setEnabled(enabled)
                self.agent_status_label.setText(f"Agent: {'Enabled' if enabled else 'Disabled'}")
                
                if enabled:
                    # If enabling, restore workflow display
                    if current_workflow:
                        self.workflow_status.setText(f"Workflow: {current_workflow}")
                        self.workflow_status.setStyleSheet("color: green;")
                        # Set the combobox to match the current workflow
                        for i in range(self.workflow_combobox.count()):
                            if self.workflow_combobox.itemText(i).lower().startswith(current_workflow):
                                self.workflow_combobox.setCurrentIndex(i)
                                break
                else:
                    # If disabling, just gray out the workflow status but preserve the text
                    current_text = self.workflow_status.text()
                    self.workflow_status.setText(current_text)
                    self.workflow_status.setStyleSheet("color: gray;")
                
                logger.info(f"Agent mode toggled: {'Enabled' if enabled else 'Disabled'}")
            else:
                # Revert checkbox state if update failed
                self.agent_toggle.setChecked(not enabled)
                QMessageBox.warning(self, "Agent Mode Error", 
                                "Failed to update agent mode. Check the logs for details.")
                
        except Exception as e:
            logger.error(f"Error in toggle_agent_mode: {e}")
            self.agent_toggle.setChecked(not enabled)
            QMessageBox.critical(self, "Error", f"Failed to toggle agent mode: {str(e)}")
