...
class AIChatFilesProcessing(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        # Connect analysisRequested signal to CodeAnalysis's _start_analysis method
        self.system_prompt_textedit.textChanged.connect(self.code_analysis._sync_system_prompt)
    ...
    def _sync_system_prompt(self):
        """Sync the system prompt with the backend or configuration."""
        try:
            prompt_content = self.system_prompt_textedit.toPlainText()
            save_system_prompt(prompt_content)  # Assuming this function saves the prompt
            self.config_manager.set('system_prompt', prompt_content)
            logger.debug("System prompt synced successfully.")
        except Exception as e:
            logger.error(f"Error syncing system prompt: {e}")
