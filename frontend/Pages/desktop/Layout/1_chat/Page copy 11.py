...
class Page(QWidget):
    ...
    def init_controls(self):
        ...
        self.agent_toggle.toggled.connect(self.toggle_agent_mode)  # Changed to toggled signal
    ...
    def toggle_agent_mode(self, checked):
        """Toggle the agent (developer) mode based on the QCheckBox state."""
        logger.info(f"toggle_agent_mode called with checked: {checked}")
        enabled = checked
        logger.info(f"toggle_agent_mode: enabled={enabled}")
        ...
        try:
            ...
            if success:
                ...
                self.update_status_message(f"Agent mode {'Enabled' if enabled else 'Disabled'}", is_agent_message=True)
