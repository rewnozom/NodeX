...
from modules.fast_whisper_v2.main import WhisperHub, WhisperTranscription  # Ensure proper import

class WhisperHubSingleton:
    """
    Singleton class to ensure only one instance of WhisperHub exists.
    """
    _instance = None
...
class WhisperPanel(DropdownPanel):
    def __init__(self, parent=None):
        try:
            ...
            # Retrieve the singleton instance of WhisperHub
            whisper_hub = WhisperHubSingleton.get_instance()
            self.whisper_widget = whisper_hub.whisper_tab  # Use the pre-initialized WhisperHub
            self.layout.addWidget(self.whisper_widget)
            logger.debug("WhisperPanel initialized successfully with WhisperHub")
        except Exception as e:
...
class HeaderSection(QWidget):
    ...
    def __init__(self, parent: QWidget):
        ...
        try:
            ...
            # Initialize WhisperPanel at startup to preload it
        ...
        try:
            ...
            if panel is None:
                if panel_class == WhisperPanel:
                    # Use the pre-initialized WhisperPanel
    ...
    def toggle_whisper(self) -> None:
        ...
        try:
            # Since WhisperPanel is pre-initialized, simply toggle its visibility
            if self.whisper_panel.isVisible():
                self.whisper_panel.hide()
                logger.info("Whisper panel hidden")
            else:
                self.hide_all_panels()
                self.whisper_panel.show_under_button(self.buttons['phone'])
                logger.info("Whisper panel shown")
        except Exception as e:
