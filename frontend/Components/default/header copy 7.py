...
from modules.fast_whisper_v2.main import WhisperTranscription  # Assuming WhisperTranscription is PySide6-based
from .terminal_widget import TerminalWidget  # Import the PySide6 TerminalWidget
...
class WhisperPanel(DropdownPanel):
    """Panel for Whisper Transcription."""
    def __init__(self, parent=None):
        try:
            super().__init__(parent, width=400, height=500)
            # Instantiate WhisperTranscription (PySide6-based)
            self.whisper_widget = WhisperTranscription()
            self.layout.addWidget(self.whisper_widget)
            logger.debug("WhisperPanel initialized successfully with WhisperTranscription")
        except Exception as e:
...
class TerminalPanel(DropdownPanel):
    ...
    def __init__(self, parent=None):
        try:
            super().__init__(parent, width=800, height=600)  # Adjust size as needed
            # Instantiate TerminalWidget (PySide6-based)
            self.terminal_widget = TerminalWidget()
            self.layout.addWidget(self.terminal_widget)
            logger.debug("TerminalPanel initialized successfully with TerminalWidget")
        except Exception as e:
...
class TerminalTabPanel(DropdownPanel):
    def __init__(self, parent=None):
        try:
            super().__init__(parent, width=800, height=600)
            self.terminal_widget = TerminalWidget()
            self.layout.addWidget(self.terminal_widget)
            logger.debug("TerminalTabPanel initialized successfully with TerminalWidget")
        except Exception as e:
            logger.error(f"Error initializing TerminalTabPanel: {e}")
            raise
...
class HeaderSection(QWidget):
    ...
    def __init__(self, parent: QWidget):
        ...
        try:
            ...
            # Initialize Panels at startup to preload them
            self.whisper_panel = WhisperPanel(self)
            logger.debug("WhisperPanel pre-initialized at startup")
            
            self.terminal_panel = TerminalPanel(self)
            logger.debug("TerminalPanel pre-initialized at startup")
        except Exception as e:
    ...
    def _verify_configuration(self) -> None:
        ...
        try:
            ...
            for config_key in required_configs:
                if config_key not in self.parent.config:
    ...
    def _setup_button_connections(self) -> None:
        ...
        try:
            ...
            self.buttons['terminal'].clicked.connect(self.toggle_terminal)  # Terminal button
            self.buttons['whisper'].clicked.connect(self.toggle_whisper)    # Whisper button
    ...
    def toggle_whisper(self) -> None:
        ...
        try:
            self.toggle_panel('whisper_panel', WhisperPanel, self.buttons['whisper'])
